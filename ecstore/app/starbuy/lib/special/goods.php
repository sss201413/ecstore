...
class starbuy_special_goods {
    ...
    function check_special_product($product_id,&$msg){
        $special_goods_mdl = app::get('starbuy')->model('special_goods');
        $filter = array(
            'status'=>'true',
    ...
    function is_delete($goods,$product_id=null){
        if($product_id){
            $check_result = $this->check_special_product($product_id,$msg);
            if(!$check_result){
                $this->error_msg = $msg;
                return false;
            }
    ...
    function check_special_product_marketable($goods_ids,&$msg){

        $products = app::get('b2c')->model('products')->getList('product_id',$goods_ids);
        if($products){
            foreach($products as $key=>$value){
                $check_result = $this->check_special_product($value['product_id'],$msg);
                if(!$check_result){
                    return false;
                    break;
                }
