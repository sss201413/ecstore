...
/*-----------------------以上为编辑货品显示数据处理函数-----------------------------*/
    ...
    private function _get_active_products($goods_id){
        ...
        foreach( kernel::servicelist("b2c_allow_delete_goods") as $object ) {
            ...
            foreach($productsData as $k=>$val){
                if( !$object->is_delete($goods_id,$val['product_id']) ){
                    ...
                    $activeServerProducts[$val['product_id']] = $object->error_msg;
                }
            }
        }
        ...
        $return = array(
            ...
            'activeServerProducts' => $activeServerProducts,
        );
        return $return;
    }
 ...
 /*--------------------------以下为保存货品函数--------------------------------------------*/
    ...
    private function _check_product($goods_id,$products){
        ...
        $activeProducts = $this->_get_active_products($goods_id);
        foreach((array)$activeProducts['activeOrderProducts']  as $pid){
            if(!$productsIds[$pid]){
                $code = 1004;
        ...
        foreach((array)$activeProducts['activeServerProducts']  as $pid=>$msg){
            if(!$productsIds[$pid]){
                $code = false;
                $msg = '货品ID ：'. $pid .' ; '. $msg;
                $this->result_error($code,$msg);
